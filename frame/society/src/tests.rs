...
fn suspended_member_life_cycle_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_noop!(Society::judge_suspended_member(Origin::signed(20), 20, true), Error::<Test, _>::NotFromSociety);

		// Society can judge thee
		// Society forgives the suspended member
		assert_ok!(Society::judge_suspended_member(Origin::signed(Society::account()), 20, true));
		assert_eq!(<SuspendedMembers<Test>>::get(20), false);
		assert_eq!(<Members<Test>>::get(), vec![10, 20]);
		...
		assert_ok!(Society::judge_suspended_member(Origin::signed(Society::account()), 20, false));
		// Cleaned up
		assert_eq!(<SuspendedMembers<Test>>::get(20), false);
		assert_eq!(<Members<Test>>::get(), vec![10]);
...
fn suspended_candidate_rejected_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_noop!(
			...
			Error::<Test, _>::NotFromSociety
		);

		// Society makes no direct judgement
		assert_ok!(Society::judge_suspended_candidate(Origin::signed(Society::account()), 20, Judgement::Rebid));
		...
		assert_ok!(Society::judge_suspended_candidate(Origin::signed(Society::account()), 20, Judgement::Reject));
		// User is slashed
		assert_eq!(Balances::free_balance(20), 25);
		assert_eq!(Balances::reserved_balance(20), 0);
...
fn max_limits_work() {
	EnvBuilder::new().with_pot(100000).execute(|| {
		...
		assert_ok!(Society::set_max_members(Origin::signed(Society::account()), 200));
		// Rotate period
		run_to_block(16);
		// Candidates are back!
		assert_eq!(Society::candidates().len(), 10);
...
fn bidding_action_works() {
	...
	EnvBuilder::new().execute(|| {
		...
		assert_ok!(Balances::transfer(Origin::signed(50), Society::account(), 25));
		assert_eq!(Balances::free_balance(Society::account()), 25);
		...
		assert_eq!(Balances::free_balance(Society::account()), 10);
		...
		assert_noop!(
			Society::judge_suspended_candidate(Origin::signed(Society::account()), 10, Judgement::Approve),
...
fn bid_judge_actions_works() {
	...
	EnvBuilder::new().execute(|| {
		...
		// 10 makes an action bid that forgives candidate 50
		let boxed_call = Box::new(call_judge_suspended_candidate(50, Judgement::Approve));
		assert_ok!(Society::bid_action(Origin::signed(10), 50, boxed_call.clone()));
		...
		// but 50 can
		assert_ok!(Society::bid(Origin::signed(50), 200));
		...
		assert_eq!(
			Society::candidates(),
			vec![
				create_bid(50, 30, BidKind::Action(50, boxed_member_call.clone())),
			]
		);
