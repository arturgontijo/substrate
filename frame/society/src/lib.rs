...
pub mod pallet {
	...
	pub trait Config<I: 'static = ()>: frame_system::Config {
		...
		type Call: Parameter
			...
			+ IsSubType<frame_system::Call<Self>>
	...
	pub enum Error<T, I = ()> {
		...
		/// The caller is not the Society account.
		NotFromSociety,
	}
	...
	pub enum Event<T: Config<I>, I: 'static = ()> {
		...
		Strike(T::AccountId, T::AccountId),
	}
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn judge_suspended_member(
			...
			ensure!(society_account == Self::account(), Error::<T, I>::NotFromSociety);
		...
		/// Allow society to make judgement on a suspended candidate.
		///
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	fn rotate_period(members: &mut Vec<T::AccountId>) {
		...
		if !members.is_empty() {
			...
			let accepted = candidates
				...
				.filter_map(|Bid { value, who: candidate, kind }| {
					...
					let bad_vote = |v: Vote, m: &T::AccountId| {
						...
						// payout and increase their strikes only if skeptic.
						// After MaxStrikes then they go into suspension.
						let amount = Self::slash_payout(m, T::WrongSideDeduction::get());

						let mut strikes = <Strikes<T, I>>::get(m);
						if v == Vote::Skeptic {
							strikes += 1;
							Self::deposit_event(Event::Strike(candidate.clone(), m.clone()));
					...
					if is_accepted {
						...
						if let BidKind::Action(_, _) = kind {
							...
							Self::deposit_event(Event::CandidateSuspended(candidate));
							None
						}
					}
				})
			...
			// account.
			let unaccounted = T::Currency::free_balance(&Self::treasury()).saturating_sub(pot);
	...
	fn execute_accepted_candidate(
		...
		let value = match kind {
			...
			BidKind::Action(deposit, call) => {
				...
				let society_account = Self::account();
	...
	/// The account ID of the society. This is used to perform all bid_actions calls.
	///
	/// This actually does computation. If you need to keep using it, then make sure you cache the
	/// value and only call this once.
	pub fn account() -> T::AccountId {
		T::PalletId::get().into_sub_account(b"society")
	}
...
impl<T: Config<I>, I: 'static> OnUnbalanced<NegativeImbalanceOf<T, I>> for Pallet<T, I> {
	fn on_nonzero_unbalanced(amount: NegativeImbalanceOf<T, I>) {
		...
		let _ = T::Currency::resolve_creating(&Self::treasury(), amount);
